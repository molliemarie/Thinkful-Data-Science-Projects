...
# Change values from strings to float number values
df['Alcohol'] = df['Alcohol'].astype(float)
df['Tobacco'] = df['Tobacco'].astype(float)

# Find mean, median, mode, range, variance, and standard deviation of Alcohol and Tobacco data
...
# Simple Ranking:
simple_ranking_tobacco = ss.rankdata(df.Tobacco)
simple_ranking_alcohol = ss.rankdata(df.Alcohol)
print("The simple rankings of the tobacco values are", simple_ranking_tobacco)
print("The simple rankings of the alcohol values are", simple_ranking_alcohol)

# Percentile Ranking (percent of data points that are below the data point):
percentile_ranking_tobacco = []
for x in df.Tobacco:
	per = stats.percentileofscore(df.Tobacco, x)
	percentile_ranking_tobacco.append(per)

percentile_ranking_alcohol = []
for x in df.Alcohol:
	per = stats.percentileofscore(df.Alcohol, x)
	percentile_ranking_alcohol.append(per)

print("The percentile rankings of the tobacco values are", percentile_ranking_tobacco)
print("The percentile rankings of the alcohol values are", percentile_ranking_alcohol)

# Zscores (number of Standard devs that a data point is from the mean):
zscore_tobacco = []
for x in df.Tobacco:
...
print("The z-score rankings of tobacco values are", zscore_tobacco)
print("The z-score rankings of tobacco values are", zscore_alcohol)
